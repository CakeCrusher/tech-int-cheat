...
chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
    ...
    console.log('CURRENT_CHAT', request.data);
    if (request.type === 'CURRENT_CHAT') {
        ...
        passedChat.forEach((passedChatInstance) => {
            const chatIndex = chat.findIndex(
                (chatInstance) =>
                    chatInstance.ticId === passedChatInstance.ticId,
            );
            if (chatIndex > -1) {
        ...
        chrome.tabs.query({}, function (tabs) {
            tabs.forEach((tab) => {
                const url = tab.url;
                if (request.clientType === 'GOOGLE MEET') {
                    const meetId = url.split('?')[0].split('/').pop();
                    console.log('sending current chat to tabs', meetId, chats);
                    chrome.tabs.sendMessage(tab.id, {
                        type: 'FULL_CURRENT_CHATS',
                        data: { chats, meetId },
                        clientType: 'GOOGLE MEET',
                    });
                } else if (request.clientType === 'ZOOM') {
                    console.log(request.data);
                    const meetId = url.split('/')[4];
                    chrome.tabs.sendMessage(tab.id, {
                        type: 'FULL_CURRENT_CHATS',
                        data: { chats, meetId },
                        clientType: 'ZOOM',
